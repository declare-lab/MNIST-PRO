...
def _retry(fn, label, max_retries=DEFAULT_MAX_RETRIES):
    ...
    for attempt in range(max_retries):
        try:
            ...
            last = exc
            print(
                f"[{label}] attempt {attempt + 1}/{max_retries} failed: {exc}. "
                f"Retrying in {min(delay, MAX_DELAY)}s...",
                flush=True,
            )
    ...
    for attr in ("model_dump",):
        if hasattr(usage, attr):
            try:
                return {
                    k: v
                    for k, v in getattr(usage, attr)().items()
...
class GCPBackend:
    def __init__(
        ...
        project_id="gcp-eee-declare-48e6",
        region="global",
    ):
...
class DeepSeekBackend:
    ...
    def __init__(
        self, model_name="deepseek-chat", base_url="https://api.deepseek.com/v1"
    ):
        from openai import OpenAI
