...
def cmd_harness(args):
    ...
    for report in availability_report():
        ...
        print(
            f"  memory controlled: {'yes' if spec.controls_memory else 'no'}"
...
def cmd_analyse(args):
    ...
    for digits in sorted({r.cell.digits for r in results}):
        ...
        for row in main_table(results, digits=digits):
            ...
            steps = (
                "--" if row["average_steps"] is None else f"{row['average_steps']:.2f}"
            )
            print(
                f"  {row['model']:<24} {row['memory']:<22} H={row['horizon']:<3} "
...
def build_parser():
    p = argparse.ArgumentParser(
        prog="mnist-pro", description="Active-glimpse evaluation framework"
    )
    ...
    r.add_argument("--memory", default="textual_state")
